...
class D4RTSceneRepresentation:
    ...
    _frames_gray: Optional[List[np.ndarray]] = field(default=None, repr=False)
    _flow_engine: Optional[object] = field(default=None, repr=False)

    def get_flow_fullres(self, frame_idx: int) -> Optional[np.ndarray]:
        """Get dense flow at full resolution. Computes lazily on-demand if not cached."""
        if frame_idx not in self.dense_flows:
            # Lazy computation — only compute when actually needed
            if (self._frames_gray is not None and self._flow_engine is not None
                    and 0 <= frame_idx < len(self._frames_gray) - 1):
                h, w = self.image_size
                if self.flow_scale < 1.0:
                    h_s = int(h * self.flow_scale)
                    w_s = int(w * self.flow_scale)
                    small_prev = cv2.resize(self._frames_gray[frame_idx],
                                            (w_s, h_s), interpolation=cv2.INTER_AREA)
                    small_curr = cv2.resize(self._frames_gray[frame_idx + 1],
                                            (w_s, h_s), interpolation=cv2.INTER_AREA)
                    flow = self._flow_engine.dense_flow(small_prev, small_curr)
                else:
                    flow = self._flow_engine.dense_flow(
                        self._frames_gray[frame_idx],
                        self._frames_gray[frame_idx + 1])
                self.dense_flows[frame_idx] = flow
            else:
                return None
...
def _make_grid_points(w: int, h: int, n_points: int,
                      margin: int = 10) -> np.ndarray:
    """Create a uniform grid of ~n_points covering the frame.

    Faster than goodFeaturesToTrack and guarantees uniform spatial coverage,
    ensuring features in all regions (including the center) are tracked
    from the very first frame.
    """
    aspect = w / h
    n_cols = max(2, int(np.sqrt(n_points * aspect)))
    n_rows = max(2, int(n_points / n_cols))
    xs = np.linspace(margin, w - margin, n_cols, dtype=np.float32)
    ys = np.linspace(margin, h - margin, n_rows, dtype=np.float32)
    xg, yg = np.meshgrid(xs, ys)
    return np.stack([xg.ravel(), yg.ravel()], axis=1)


def _write_ply(path: str, points: np.ndarray, colors: np.ndarray):
    """Write PLY file using batch numpy operations (much faster than per-point loop)."""
    n = len(points)
    header = (f"ply\nformat ascii 1.0\nelement vertex {n}\n"
              f"property float x\nproperty float y\nproperty float z\n"
              f"property uchar red\nproperty uchar green\n"
              f"property uchar blue\nend_header\n")
    data = np.column_stack([points, colors.astype(int)])
...
class D4RTEncoder:
    ...
    def encode(self, frames_gray: List[np.ndarray],
               flow_scale: float = 0.25) -> D4RTSceneRepresentation:
        """
        Encode video into scene representation (lazy mode).

        Dense optical flows are NOT precomputed upfront — they are computed
        on-demand when requested by compute_flow() or answer_single_query().
        This saves ~2-4 minutes at full resolution since most demos only
        need LK tracking, not dense flow.

        Args:
            frames_gray: list of grayscale frames
            flow_scale: resolution scale for on-demand dense flow computation
                        (0.25 = quarter-res, very fast). Does NOT affect LK
                        tracking quality since tracking uses full-res frames.
        """
        n = len(frames_gray)
        h, w = frames_gray[0].shape[:2]

        print(f"  [Encoder] Lazy mode: {n} frames at {w}x{h}, "
              f"flows computed on-demand at scale={flow_scale}")
        ...
        scene = D4RTSceneRepresentation(
            ...
            _frames_gray=frames_gray,
            _flow_engine=self.flow_engine,
        )
        return scene
...
class D4RTQueryDecoder:
    ...
    def track_points_batched(self, points: np.ndarray,
        ...
        for t_rel in range(1, n_frames):
            ...
            # Backward consistency check (skip periodically for speed —
            # saves ~33% of LK calls with minimal quality loss)
            do_fb = (t_rel % 3 == 0) or (t_rel <= 5)
            if do_fb:
            ...
            if do_fb:
                ...
                fb_errors = np.zeros(n_alive, dtype=np.float32)
                fb_check = np.zeros(n_alive, dtype=bool)
                fb_fail = np.zeros(n_alive, dtype=bool)
            ...
            if len(good_global) > 0:
                ...
                # Carry forward confidence; apply FB decay when checked
                confidences[good_global, t_rel] = \
                    confidences[good_global, t_rel - 1]
                if do_fb:
...
class D4RT:
    ...
    def compute_flow(self, frame_a: int, frame_b: int) -> np.ndarray:
        ...
        For adjacent frames: computes on-demand (lazy) or returns cached flow.
        For non-adjacent: chains flows sequentially.
        """
        self._check_loaded()
        ...
        if frame_b == frame_a + 1:
            flow = self.scene.get_flow_fullres(frame_a)
            if flow is not None:
                return flow.copy()
    ...
    def track_rolling_window(self, n_points: int = 200,
        ...
        class TrackedPoint:
            ...
            def __init__(self, pid, pos, color):
                ...
                self.age = 0
                self.color = color
                self.grace = 0  # frames of grace before killing
                self.origin = pos.copy()  # grid position it spawned at
                self.moved = False  # has it genuinely left its origin?

        def make_color(pos):
            """Color based on spatial origin: x->hue, y->value."""
            hue = int(180 * pos[0] / max(w, 1))
            val = 180 + int(75 * pos[1] / max(h, 1))
            c = cv2.cvtColor(np.uint8([[[hue, 255, val]]]),
                             cv2.COLOR_HSV2BGR)[0][0]
            return tuple(int(x) for x in c)

        # Initialize from uniform grid (guaranteed spatial coverage)
        grid_points = _make_grid_points(w, h, n_points)
        min_dist = max(5, int(min(w, h) / (n_points ** 0.5) * 0.6))

        points_list = []
        for pt in grid_points:
            points_list.append(TrackedPoint(next_pid, pt, make_color(pt)))
            next_pid += 1
        ...
        for t_rel in range(n_frames):
            ...
            if t_rel > 0 and t_abs < len(self.frames_gray):
                ...
                if alive_pts_list:
                    ...
                    prev_positions = prev_arr.reshape(-1, 2)

                    # Vectorized FB errors
                    fb_errors = np.linalg.norm(
                        prev_positions - back_arr.reshape(-1, 2), axis=1)

                    # Per-frame displacement (for jitter suppression)
                    frame_disp = np.linalg.norm(
                        new_positions - prev_positions, axis=1)
                    ...
                    new_in_bounds = (
                        ...
                        (new_positions[:, 1] >= 0) &
                        (new_positions[:, 1] < h))
                    # Tighter FB consistency: 3px (was 8px)
                    fb_ok = (back_st_f == 1) & (fb_errors <= 3.0)

                    good = fwd_ok & new_in_bounds & fb_ok
                    acceptable = fwd_ok & new_in_bounds & ~fb_ok
                    out_of_bounds = fwd_ok & ~new_in_bounds

                    # Jitter threshold: movements below this are noise
                    JITTER_THRESH = 0.8  # pixels
                    # Max plausible per-frame displacement — anything beyond
                    # this is a bad LK match (lighting change, etc.)
                    MAX_DISP = 15.0  # pixels per frame
                    # Distance from origin to count as "genuinely moved"
                    MOVE_THRESH = 3.0  # pixels

                    for j, p in enumerate(alive_pts_list):
                        # --- Reset to origin if LK teleported the point ---
                        if fwd_ok[j] and frame_disp[j] > MAX_DISP:
                            p.pos = p.origin.copy()
                            p.trail = [p.origin.copy()]
                            p.moved = False
                            p.grace = 0
                            p.age += 1
                            continue
                        ...
                        if good[j]:
                            # Jitter suppression: if displacement is tiny,
                            # snap back to previous position (noise)
                            if frame_disp[j] < JITTER_THRESH:
                                p.age += 1
                                p.grace = 0
                                continue
                            ...
                            # Check if point has genuinely left its origin
                            origin_dist = np.linalg.norm(p.pos - p.origin)
                            if origin_dist > MOVE_THRESH:
                                p.moved = True

                        elif acceptable[j]:
                            # FB failed — only accept if displacement is real
                            if frame_disp[j] < JITTER_THRESH:
                                p.age += 1
                                continue
            ...
            # --- Replace only at vacated grid positions ---
            # A grid position needs replacement only when no alive point
            # is still near it (i.e. the original point moved away or died)
            if t_rel > 0 and t_rel % replace_every == 0:
                alive_pts = [p for p in points_list if p.alive]
                alive_count = len(alive_pts)
                if alive_count > 0:
                    alive_positions = np.array(
                        [p.pos for p in alive_pts], dtype=np.float32)
                else:
                    alive_positions = np.empty((0, 2), dtype=np.float32)

                # Find grid positions that are vacated (no alive point nearby)
                new_pts = []
                for gp in grid_points:
                    if len(alive_positions) > 0:
                        dists = np.linalg.norm(alive_positions - gp, axis=1)
                        if dists.min() < min_dist:
                            # An alive point is still covering this grid slot
                            continue
                    # This grid position is uncovered — re-seed it
                    new_pts.append(gp)

                if new_pts:
                    # Remove dead points from list to avoid unbounded growth
                    points_list = [p for p in points_list if p.alive]
                    for pt in new_pts:
                        points_list.append(TrackedPoint(
                            next_pid, pt, make_color(pt)))
                        next_pid += 1
                        total_born += 1
            ...
            if writer or show:
                ...
                # Draw grid positions as faint colored markers (shows where
                # new points spawn, colored by spatial origin)
                grid_dot = max(1, w // 600)
                for gp in grid_points:
                    gx, gy = int(gp[0]), int(gp[1])
                    gc = make_color(gp)
                    # Dim the grid color to ~40% brightness
                    gc_dim = tuple(c // 3 for c in gc)
                    cv2.circle(vis, (gx, gy), grid_dot,
                               gc_dim, -1)

                # Draw trails + points
                line_thick = max(1, w // 500)
                dot_radius = max(1, w // 400)
                dot_radius_static = max(1, w // 600)
                for p in points_list:
                    if not p.alive:
                        continue
                    if p.moved:
                        # Moving point: draw trail + bright dot
                        if len(p.trail) > 1:
                            pts_arr = np.array(p.trail, dtype=np.int32)
                            cv2.polylines(vis, [pts_arr], False, p.color,
                                          line_thick)
                        cv2.circle(vis, (int(p.pos[0]), int(p.pos[1])),
                                   dot_radius, p.color, -1)
                    else:
                        # Stationary point: small dot only (no trail noise)
                        cv2.circle(vis, (int(p.pos[0]), int(p.pos[1])),
                                   dot_radius_static, p.color, -1)
    ...
    def visualize_tracking(self, points: np.ndarray,
        ...
        # Color each point based on its spatial origin (x->hue, y->value)
        colors = []
        for i in range(num_points):
            pt = points[i]
            hue = int(180 * pt[0] / max(w, 1))
            val = 180 + int(75 * pt[1] / max(h, 1))
            color_hsv = np.uint8([[[hue, 255, val]]])
            color_bgr = cv2.cvtColor(color_hsv, cv2.COLOR_HSV2BGR)[0][0]
            colors.append(tuple(int(c) for c in color_bgr))
        trail_length = 60  # only show last 60 frames of trail
        ...
        for frame_idx in range(start_frame, end_frame):
            ...
            for pt_idx, track in enumerate(all_tracks):
                ...
                if not occ and conf > 0.3:
                    # Draw trailing polyline (last trail_length frames)
                    t_start = max(0, t_rel - trail_length)
                    trail_pts = []
                    for k in range(t_start, t_rel + 1):
                        if track['occluded'][k] or track['confidences'][k] <= 0.3:
                            # Gap in trail — draw what we have and restart
                            if len(trail_pts) > 1:
                                arr = np.array(trail_pts, dtype=np.int32)
                                cv2.polylines(vis, [arr], False,
                                              colors[pt_idx], line_thick)
                            trail_pts = []
                        else:
                            p = track['positions_2d'][k]
                    ...
                    # Draw current dot
                    cv2.circle(vis, (int(pos[0]), int(pos[1])),
                               dot_radius, colors[pt_idx], -1)
...
if __name__ == "__main__":
    ...
    for seg_idx, (seg_start, seg_end) in enumerate(segments):
        ...
        # Grid-based point initialization: uniform spatial coverage,
        # guaranteed to track center-of-frame features from the start
        points = _make_grid_points(w, h, 2000)
        print(f"  Tracking {len(points)} grid points across {n_frames} frames...")
        ...
        if len(points) > 0:
            ...
            try:
                ...
                axes[0].imshow(cv2.cvtColor(model.frames[seg_start],
                                            cv2.COLOR_BGR2RGB))
                # Color each track by its spatial origin
                img_h, img_w = model.scene.image_size
                def _origin_color(pt):
                    """Map origin (x,y) to matplotlib RGB color via HSV."""
                    import colorsys
                    hue = pt[0] / max(img_w, 1)
                    val = 0.7 + 0.3 * pt[1] / max(img_h, 1)
                    return colorsys.hsv_to_rgb(hue, 1.0, val)

                for i, tr in enumerate(tracks):
                    c = _origin_color(points[i])
                    valid = ~tr['occluded']
                    pos = tr['positions_2d'][valid]
                    if len(pos) > 1:
                        axes[0].plot(pos[:, 0], pos[:, 1], '-', color=c,
                                     linewidth=1, alpha=0.8)
                        axes[0].plot(pos[0, 0], pos[0, 1], 'o', color=c,
                                     markersize=5)
                ...
                for i, tr in enumerate(tracks):
                    ...
                    axes[1].plot(t_axis, diffs, color=c,
        ...
        _write_ply(ply_path, pts3d, colors_3d)
        ...
        if n_frames >= 10:
            ...
            for kf_frame, cloud in sorted(recon4d['keyframe_clouds'].items()):
                ...
                _write_ply(kf_ply, cloud['points_3d'], cloud['colors'])
        ...
        if n_frames >= 20:
            ...
            rw_result = model.track_rolling_window(
                ...
                trail_length=60, output_path=rw_path, show=show
            )
